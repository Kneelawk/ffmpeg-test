...
public class FfmpegTest {
    public static void main(String[] args) {
        ...
        try (Arena confined = Arena.ofConfined()) {
            ...
            int videoIndex = avformat_h.av_find_best_stream(ctx, avutil_h.AVMEDIA_TYPE_VIDEO(), -1, -1, videoCodecRef, 0);
            int audioIndex = avformat_h.av_find_best_stream(ctx, avutil_h.AVMEDIA_TYPE_AUDIO(), -1, -1, audioCodecRef, 0);
            ...
            if (audioIndex < 0) {
                System.out.println("Contains no audio streams.");
                return;
            }

            MemorySegment videoCodec = videoCodecRef.get(ValueLayout.ADDRESS.withTargetLayout(AVCodec.$LAYOUT()), 0);
            MemorySegment audioCodec = audioCodecRef.get(ValueLayout.ADDRESS.withTargetLayout(AVCodec.$LAYOUT()), 0);

            MemorySegment streams = AVFormatContext.streams$get(ctx);
            MemorySegment videoStream = streams.getAtIndex(ValueLayout.ADDRESS.withTargetLayout(AVStream.$LAYOUT()), videoIndex);
            MemorySegment audioStream = streams.getAtIndex(ValueLayout.ADDRESS.withTargetLayout(AVStream.$LAYOUT()), audioIndex);
